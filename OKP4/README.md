...
## Values in "" remain in ""
```
Example: okp4d keys add <wallet name> --> okp4d keys add wallet
...
```
cd $HOME && git clone https://github.com/okp4/okp4d.git && \
cd okp4d && \
git checkout v3.0.0 && \
make install
```
## Init node
```
okp4d init <Node Name> --chain-id okp4-nemeton-1
...
indexer="null" && \
min_retain_blocks=1 && \
minimum_gas_prices="0.0025uknow" && \
...
sed -i.bak -e "s/^indexer *=.*/indexer = \"$indexer\"/" /root/.okp4d/config/config.toml && \
sed -i.bak -e "s/^min-retain-blocks *=.*/min-retain-blocks = \"$min_retain_blocks\"/" /root/.okp4d/config/app.toml && \
sed -i.bak -e "s/^minimum-gas-prices *=.*/minimum-gas-prices = \"$minimum_gas_prices\"/" /root/.okp4d/config/app.toml && \
sed -i.bak -e "s/^snapshot-interval *=.*/snapshot-interval = \"$snapshot_interval\"/" /root/.okp4d/config/app.toml && \
sed -i.bak -e "s/^pruning *=.*/pruning = \"$pruning\"/" /root/.okp4d/config/app.toml && \
sed -i.bak -e "s/^pruning-keep-recent *=.*/pruning-keep-recent = \"$pruning_keep_recent\"/" /root/.okp4d/config/app.toml && \
sed -i.bak -e "s/^pruning-keep-every *=.*/pruning-keep-every = \"$pruning_keep_every\"/" /root/.okp4d/config/app.toml && \
sed -i.bak -e "s/^pruning-interval *=.*/pruning-interval = \"$pruning_interval\"/" /root/.okp4d/config/app.toml
sed -i.bak -e "s/^min-retain-blocks *=.*/min-retain-blocks = \"$min_retain_blocks\"/" /root/.okp4d/config/app.toml
sed -i.bak -e "s/^inter-block-cache *=.*/inter-block-cache = \"$inter_block_cache\"/" /root/.okp4d/config/app.toml
```
## Download addrbook
```
...
-y
```
### Important save priv_validator_key.json file from .okp4d/config/
